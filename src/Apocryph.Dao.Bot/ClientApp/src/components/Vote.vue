			...
			<fieldset>
				...
				<div class="form-group">
					<div class="button-grid">
						<button class="btn btn-default btn-ghost" role="button" name="voteNo" id="votePass"
							v-if="showVotingButtons" @click="vote(0, $event)">
							Pass
						</button>

						<button class="btn btn-primary" role="button" name="voteYes" id="voteYes"
							v-if="showVotingButtons" @click="vote(1, $event)">
							Yes
						</button>

						<button class="btn btn-error" role="button" name="voteNo" id="voteNo" v-if="showVotingButtons"
							@click="vote(2, $event)">
							No
						</button>

						<button class="btn btn-primary" role="button" name="enact" id="enact" v-if="showEnactButton"
							@click="enact($event)">
							Enact
						</button>
					</div>
		...
		methods: {
			async vote(option, e) {
				...
				try {
					...
					await contract.vote(vm.voteId, option)
					vm.success = true;
				} catch (err) {
			...
			async enact(e) {
				...
				try {
					...
					await contract.enact(vm.voteId);
					vm.success = true;
				} catch (err) {
					console.error(err);
					vm.error = true;
			...
			async initButtons() {
				...
				vm.isVotable = await contract.isWithinVoteDeadline(vm.voteId)
				vm.isEnactable = await contract.isAfterEnactDelay(vm.voteId)
			},
			async initVm() {
				const vm = this;
